		...
		if p4 ~= l__Torso__1 then
			if (v4 - v5).magnitude < 80 then
				...
				if 16 < math.abs(l__Torso__1.Position.y - p4.Position.y) then
					v6.BackgroundColor3 = Color3.new(v6.BackgroundColor3.r * 0.5, v6.BackgroundColor3.g * 0.5, v6.BackgroundColor3.b * 0.5);
				elseif 32 < math.abs(l__Torso__1.Position.y - p4.Position.y) then
					v6.BackgroundColor3 = Color3.new(v6.BackgroundColor3.r * 0.2, v6.BackgroundColor3.g * 0.2, v6.BackgroundColor3.b * 0.2);
				else
					v6.ZIndex = 3;
					v6.Direction.ZIndex = 3;
				end;
				v6.Position = UDim2.new(u3.X.Scale + D(v4.x, v5.x) / 80 / 2, -5, u3.Y.Scale + D(v4.z, v5.z) / 80 / 2, -5);
	...
	if not p4.Parent:findFirstChild("Humanoid") then
		if p4 ~= l__Torso__1 then
			...
			if 14 < l__Torso__1.Position.y - p4.Position.y then
				v7.Title.Text = "(BELOW)";
				v7.BackgroundColor3 = Color3.new(v7.BackgroundColor3.r * 0.5, v7.BackgroundColor3.g * 0.5, v7.BackgroundColor3.b * 0.5);
			elseif 28 < l__Torso__1.Position.y - p4.Position.y then
				v7.Title.Text = "(FAR BELOW)";
				v7.BackgroundColor3 = Color3.new(v7.BackgroundColor3.r * 0.2, v7.BackgroundColor3.g * 0.2, v7.BackgroundColor3.b * 0.2);
			...
			if (v4 - v5).magnitude < 80 then
				v7.Position = UDim2.new(u3.X.Scale + D(v4.x, v5.x) / 80 / 2, -5, u3.Y.Scale + D(v4.z, v5.z) / 80 / 2, -5);
			else
				local v11 = v5 - (v5 - v4).unit * 80;
				v7.Position = UDim2.new(u3.X.Scale + D(v11.x, v5.x) / 80 / 2, -5, u3.Y.Scale + D(v11.z, v5.z) / 80 / 2, -5);
			end;
			coroutine.resume(coroutine.create(function()
				waitTime(2);
				v7:Destroy();
...
while true do
	...
	waitTime(2);
	local v20, v21, v22 = l__Torso__1.CFrame:toEulerAnglesXYZ();
